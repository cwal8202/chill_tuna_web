...
# chat.models 가 있으면 스레드 히스토리를 DB에서 읽어옵니다.
try:
    from chat.models import ChatMessage, ChatThread  # noqa
    HAS_CHAT_MODELS = True
except Exception:
    ChatMessage = ChatThread = None  # type: ignore
...
def _load_history_text(thread_id: Optional[int], limit: int = 10, char_limit: int = 1800) -> str:
    """최근 발화 limit개를 '사용자:… / 페르소나:…' 형식으로 합쳐 문자열 반환"""
    ...
    lines: List[str] = []
    for m in items:
        who = "사용자" if m.sender != "persona" else "페르소나"
...
def _load_history_messages(thread_id: Optional[int], limit: int = 16) -> List[Dict[str, str]]:
    """OpenAI chat.completions 포맷으로 최근 메시지 반환 (오래된→최신)"""
    if not (HAS_CHAT_MODELS and thread_id):
        return []
    qs = (
        ChatMessage.objects.filter(thread_id=thread_id)
    ...
    for m in items:
        role = "assistant" if m.sender == "persona" else "user"
        out.append({"role": role, "content": m.message})
    return out


# ==============================
# 스코프/연속성 게이트 (식품 전용, LLM 우선 + 휴리스틱 백업)
# ==============================
REFUSAL_TEXT = (
...
INTENT_HINTS = (
    "살래","사 말래","사말래","사야","사요","구매","몇개","몇 개","몇개나","수요","판매",
    "예측","수량","빈도","출시","프로모션","가격","할인","재구매","늘리","증가","몇 병","몇 캔",
    "어때","추천","골라","비교","대비","vs","선호","취향","낫겠","낫다","좋아","보다"
)
...
def _parse_yes_no(token: str) -> Optional[bool]:
    ...
    if YES_PAT.match(t): return True
    if NO_PAT.match(t):  return False
    return None
...
def _is_in_scope_food(user_input: str, thread_id: Optional[int], client, model_name: str) -> bool:
    ...
    if _heuristic_food_in_scope(user_input): return True
...
def _product_category_from_text(text: str) -> str:
    t = (text or "").lower()
    if any(k in t for k in (h.lower() for h in HAM_HINTS)): return "canned_ham"
    if any(k in t for k in (h.lower() for h in TUNA_HINTS)): return "canned_tuna"
    if any(k in t for k in (h.lower() for h in SAUCE_HINTS)): return "sauce"
    if any(k in t for k in (h.lower() for h in BEV_HINTS)): return "beverage"
    if any(k in t for k in (h.lower() for h in HMR_HINTS)):  return "hmr"
    return "default"

def _household_bucket_from_tag(tag: str) -> str:
    tag = (tag or "")
    if any(x in tag for x in ("3인","4인")): return "small"
    if any(x in tag for x in ("5인","6인","대가족")): return "large"
    if "2인" in tag: return "small"
    if "1인" in tag: return "single"
    return "single"
...
def _apply_quantity_guard_text(product_hint: str, persona_tag: str, response_text: str) -> str:
    ...
    if hh == "large": high = int(round(high * 1.5))

    new_n, note = n, ""
    if n > high:
        new_n = high; note = f" (일반 가정 기준, {new_n}개로 잡아 설명 드렸어요)"
    elif n < low:
        new_n = low;  note = f" (최소 사용량을 고려해 {new_n}개로 안내했어요)"
    if new_n == n: return response_text

    adjusted = FIRST_COUNT_PAT.sub(lambda _: f"{new_n}개", response_text, count=1)
    lines = adjusted.splitlines()
    if lines: lines[0] = lines[0].rstrip() + note
    return "\n".join(lines)
...
1) 첫 줄: 저는 한 달에 N개(규격 기준)'를 구매할 것 같아요! (뒤에 대괄호로 내 판단 한 줄 요약을 붙인다.)
   예) 저는 한 달에 4개(150g 2,000원 기준)를 구매할 것 같아요!
2) 내 기준(핵심 3~5줄):
...
3) 제품의 장점:
   · 한 줄씩 2~3개(예: 간편하게 먹기 좋아요)
4) 제품의 단점:
   · 한 줄씩 2~3개(예: 칼로리가 걱정될 수 있어요)
5) 이렇게 되면 더 좋아요:
   · 제품이 ~하면 더 좋아요(2~3개 제안)
- 불확실하면 '모르면 생략'. 숫자는 과장하지 말고 범위로 표현 가능.
...
def _get_persona_name(persona) -> str:
    ...
    if tag:
        token = tag.split()[0]
        if token and len(token) <= 6: return token
    return "마케팅 도우미"
...
def _maybe_handle_persona_intro(persona, user_input: str) -> Optional[str]:
    ...
    if not any(key in t for key in SELF_INTRO_TRIGGERS): return None
    name = _get_persona_name(persona); tag = getattr(persona,"persona_summary_tag","") or ""
    ...
    age = pick(ages, tag) or ""; gender = pick(genders, tag) or ""
    household = pick(households, tag) or ""; region = pick(regions, tag) or ""
    h = ("건강" in tag); c = ("편의" in tag) or ("간편" in tag) or ("hmr" in tag) or ("HMR" in tag)
    premium = ("프리미엄" in tag) or ("품질" in tag)
    low_price = ("가격" in tag and ("구애받지 않" in tag or ("민감" in tag and ("낮" in tag or "적" in tag)))) or premium
    parts: List[str] = []
    if h and c: parts.append("건강·편의 선호가 높고")
    elif h:     parts.append("건강을 특히 중시하고")
    elif c:     parts.append("편의를 특히 중시하고")
...
LIKE_TRIGGERS = ("좋아하는 음식","최애","무슨 음식 좋아","좋아하는 메뉴")
DISLIKE_TRIGGERS = ("싫어하는 음식","무슨 음식 싫어","안 좋아하는 음식")
...
def _parse_persona_traits(tag: str) -> Dict[str, float]:
    ...
    def _num_after_colon(start_idx: int) -> Optional[float]:
        ...
        j = i; allowed = "0123456789."
        while j < n and src[j] in allowed: j += 1
        try: return float(src[i:j])
        except Exception: return None
    for key in ("brand_loyalty","cooking_convenience","health_orientation","hmr_preference",
                "premium_orientation","price_sensitivity","variety_seeking"):
        kpos = src.find(key)
        if kpos != -1:
...
def _maybe_handle_food_preference(persona, user_input: str) -> Optional[str]:
    """구매의도 없을 때만 작동하는 선호/비선호/아이템 호불호 응답"""
    t = (user_input or "").strip().lower()
    if _has_purchase_intent(t):  # 핵심: 구매/수량/가격 질문이면 소토크 비활성화
        return None
    ...
    if any(k in t for k in LIKE_TRIGGERS):
        picks = []
        if health >= 0.6: picks.append("샐러드나 그릴드 같은 담백한 메뉴")
        if hmr >= 0.6:    picks.append("간단히 데워 먹는 밀키트/즉석 한 끼")
        if premium >= 0.6:picks.append("원재료가 잘 보이는 프리미엄 제품")
        if not picks:     picks.append("집에서 손쉽게 준비할 수 있는 편한 메뉴")
        extra = " 바쁠 땐 HMR도 자주 골라요." if hmr >= 0.6 else ""
        return f"저는 {', '.join(picks)}를 좋아해요.{extra}"

    if any(k in t for k in DISLIKE_TRIGGERS):
        base = "너무 달거나 기름진 음식, 짠맛이 강한 가공육" if health >= 0.6 else "특별히 가리는 건 많지 않아요"
        return f"저는 {base}는 잘 안 먹어요."

    # 아이템 호불호는 '좋아/싫어/어때' 같은 정서 키워드가 있을 때만
...
def _get_llm_response_impl(
    ...
    """
    인사/소개 → 스코프 판정 → in-scope면 수요/수량 예측, out-of-scope면 소토크(선호) → 그 외 거절
    + 가격 현실성 가드, 수량 소프트 보정
    """
    ...
    if not in_scope:
        # in-scope가 아니면 소토크(선호) 시도 → 실패 시 거절
...
def _extract_thread_id_from_request(request) -> Optional[int]:
    ...
    try:
        ...
        if m: return _to_int_or_none(m.group(1))
    except Exception: pass
    return None

def get_llm_response(*args, **kwargs):
    """
    - 신규: get_llm_response(persona_id, user_input, model_name='...', thread_id=None, ...)
    - 구형: get_llm_response(request, persona_id, user_input, model_name)
    """
